...
epsilon = 0.08; %confidence level for DRCC
T = 24;  
Z = 2; %no. of WTs
...
for r = 1:n_r
    for t = 1:T
        cons = [cons, cone( rho(r,t)*sqrtm(Sigma(:,:,r,t))*ones(Z,1),...
            sqrt(epsilon/(1-epsilon))*p_EB(r,t) )];
        cons = [cons, cone( rho(r,t)*sqrtm(Sigma(:,:,r,t))*ones(Z,1),...
            sqrt(epsilon/(1-epsilon))*(P_EB-p_EB(r,t)) )];
    end
end
...
for r = 1:n_r
    for t = 1:T
        cons = [cons, cone( lambda(r,t)*sqrtm(Sigma(:,:,r,t))*ones(Z, 1),...
            sqrt(epsilon/(1-epsilon))*q_exc(r,t))];
    end
end
% p2hh
    ...
    for t = 1:T
        ...
        for k = 1:n_P2HH_corner
                cons = [cons, cone(ksi(k, r, t)*sqrtm(Sigma(:,:,r,t))*ones(Z,1), ...
                        sqrt(epsilon/(1-epsilon))*y(k, r, t))];
                cons = [cons, cone(ksi(k, r, t)*sqrtm(Sigma(:,:,r,t))*ones(Z,1), ...
                        sqrt(epsilon/(1-epsilon))*(1-y(k, r, t)))];
        end
    end
end
%p2hh
...
for r = 1:n_r
    for t = 2:T
        cons = [cons, cone( mu(r,t)*sqrtm(Sigma(:,:,r,t))*ones(Z,1), ...
           sqrt(epsilon/(1-epsilon))*(Temp(r,t)-T_min) )];
        cons = [cons, cone( mu(r,t)*sqrtm(Sigma(:,:,r,t))*ones(Z,1), ...
           sqrt(epsilon/(1-epsilon))*(T_max-Temp(r,t)) )];   
    end
end
...
for r = 1:n_r
    ...
    cons = [cons, cone( ( (1-1/(R_eqv*C))*mu(r,t)+1/C*kappa(r,t)-1/C*lambda(r,t) ) * sqrtm(Sigma(:,:,r,t))*ones(Z,1),...
        sqrt(epsilon/(1-epsilon))*((1-1/(R_eqv*C))*Temp(r,t)+1/C*(q_p2hh(r,t)-q_exc(r,t)+1/R_eqv*Ta)-T_min  ) )];
    cons = [cons, cone( ( (1-1/(R_eqv*C))*mu(r,t)+1/C*kappa(r,t)-1/C*lambda(r,t) ) * sqrtm(Sigma(:,:,r,t))*ones(Z,1),...
        sqrt(epsilon/(1-epsilon))*(-(1-1/(R_eqv*C))*Temp(r,t)-1/C*(q_p2hh(r,t)-q_exc(r,t)+1/R_eqv*Ta)+T_max  ) )];   
end
...
for r = 1:n_r
    ...
    cons = [cons, cone( sqrtm(Sigma(:,:,r,T))*(upsilon(r,T)*ones(Z,1)),...
        sqrt(epsilon/(1-epsilon))* (m_tank-m_H2(r,T)))];
end
%converter
for r = 1:n_r
    for t = 1:T
        cons = [cons, cone( Delta(r,t)*sqrtm(Sigma(:,:,r,t))*ones(Z,1),...
            sqrt(epsilon/(1-epsilon)) * (P_converter * eta_converter - p_P2HH(r,t)) )];
    end
end
%compressor
for r = 1:n_r
    for t = 1:T
        cons = [cons, cone( gamma(r,t)*sqrtm(Sigma(:,:,r,t))*ones(Z,1),...
            sqrt(epsilon/(1-epsilon)) * (m_comp - n_H2(r,t)) )];
    end
end
% cons = [cons, P_EB == 0];
% cons = [cons, n_el == 0];
...
save('DRCC_P2HH_EB_92.mat',"obj", "n_el", "P_EB", "z","u_su","u_sd", ...
...
hold off;
saveas(gcf, 'DRCC_P2HH_EB_92', 'epsc');
...
%         sqrt(epsilon/(1-epsilon))* 2E4*(m_tank-n_H2(r,:)*ones(T,1))) ];
% end
...
%                 sqrt(epsilon/(1-epsilon)) * ( -H_CHP(l)*P*x(:,r,t)-H_trans(l)*p_trans(r,t)...
